/**
 * Content Script für mein-deal.com:
...
const CONFIG = {
    // Container-Selektor: <article id="...">
    containerSelector: "article.post-item",
    idAttribute: "id", // Die eindeutige ID ist im 'id'-Attribut
    // Button-Selektor: Der 'Zum Angebot'-Button
    buttonSelector: "div.dealbutton-content a.zum-deal",
};
...
function scanDeals() {
    ...
    deals.forEach(dealElement => {
        ...
        if (CONFIG.idAttribute === 'id') {
            ...
            // Holt den Wert aus einem data-Attribut (für den Fall, dass es mal geändert wird)
    ...
    console.log(`[MeinDeal Content] ${dealIds.length} Deals gefunden.`);
    return dealIds;
}
...
function clickDeal(dealId) {
    ...
    try {
        ...
        // Finde den Artikel-Container über das ID-Attribut (z.B. article.post-item#ID)
        ...
        setTimeout(() => {
            try {
                triggerClick(button);
                console.log(`[MeinDeal Content] Klick auf Deal-Button für ID ${dealId} ausgelöst.`);
            } catch (e) {
...
console.log("[MeinDeal Content] Script geladen und bereit.");