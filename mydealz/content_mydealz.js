...
const CONFIG = {
    containerSelector: "article.thread.cept-thread-item",
    idAttribute: "id", // Die eindeutige ID ist im 'id'-Attribut
    buttonSelector: 'a[data-t="dealLink"], button[data-t="dealLink"], button.buttonWithCode-button, a.buttonWithCode-button',
};
...
function scanDeals() {
    ...
    deals.forEach(dealElement => {
        ...
        if (CONFIG.idAttribute === 'id') {
            ...
            // Holt den Wert aus einem data-Attribut
    ...
    console.log(`[MyDealz Content] ${dealIds.length} Deals gefunden.`);
    return dealIds;
}
...
function clickDeal(dealId) {
    ...
    try {
        // Finde den Deal-Container für mydealz: article.thread.cept-thread-item#ID
        const dealContainer = document.querySelector(`${CONFIG.containerSelector}#${dealId}`);

        if (!dealContainer) {
            console.error(`Deal-Container mit ID="${dealId}" nicht gefunden.`);
            return false;
        }

        // Finde den ersten Button innerhalb des Containers
        ...
        setTimeout(() => {
            try {
                triggerClick(button);
                console.log(`[MyDealz Content] Klick auf Deal-Button für ID ${dealId} ausgelöst.`);
            } catch (e) {
...
console.log("[MyDealz Content] Script geladen und bereit.");